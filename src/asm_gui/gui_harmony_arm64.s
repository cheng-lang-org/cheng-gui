...
.extern cheng_mobile_host_harmony_width
.extern cheng_mobile_host_harmony_height
...
cheng_gui_window_create:
    ...
    adrp x0, g_cfg@PAGE
    add x0, x0, g_cfg@PAGEOFF
    cbz x19, .harmony_title_done
    str x19, [x0, #CFG_TITLE]
.harmony_title_done:
    cmp w20, #0
    ble .harmony_width_done
    str w20, [x0, #CFG_WIDTH]
.harmony_width_done:
    cmp w21, #0
    ble .harmony_height_done
...
.harmony_run_loop:
    ...
    cbz w0, .harmony_run_sleep
    ldr w1, [sp]
    add sp, sp, #96
    cmp w1, #3
    beq .harmony_run_done
    b .harmony_run_loop
.harmony_run_sleep:
    add sp, sp, #96
...
harmony_render_label:
    ...
    ldr x19, [x19]
    cbz x19, .harmony_render_done

    bl cheng_mobile_host_harmony_width
    mov w20, w0
    bl cheng_mobile_host_harmony_height
    mov w21, w0

    cmp w20, #0
    bgt .harmony_width_ok
    adrp x0, g_cfg@PAGE
    add x0, x0, g_cfg@PAGEOFF
    ldr w20, [x0, #CFG_WIDTH]
.harmony_width_ok:
    cmp w21, #0
    bgt .harmony_height_ok
    adrp x0, g_cfg@PAGE
    add x0, x0, g_cfg@PAGEOFF
...
.harmony_height_ok:
    ...
    add x0, x0, g_pixels@PAGEOFF
    ldr x0, [x0]
    cbz x0, .harmony_alloc_pixels
    adrp x1, g_width@PAGE
    add x1, x1, g_width@PAGEOFF
    ldr w1, [x1]
    cmp w1, w20
    bne .harmony_alloc_pixels
    adrp x1, g_height@PAGE
    add x1, x1, g_height@PAGEOFF
    ldr w1, [x1]
    cmp w1, w21
    bne .harmony_alloc_pixels
    b .harmony_have_pixels

.harmony_alloc_pixels:
    adrp x1, g_pixels@PAGE
    add x1, x1, g_pixels@PAGEOFF
    ldr x1, [x1]
    cbz x1, .harmony_skip_free2
    mov x0, x1
    bl free
.harmony_skip_free2:
    mov x0, x25
    bl malloc
    cbz x0, .harmony_render_done
    adrp x1, g_pixels@PAGE
    add x1, x1, g_pixels@PAGEOFF
    str x0, [x1]
    adrp x1, g_width@PAGE
...
.data
g_default_title: .asciz "Cheng ASM Harmony"
