...
static void usage(void) {
  fprintf(stdout,
          "Usage:\n"
          "  verify_r2c_equivalence_harmony_native [--project <abs>] [--entry </app/main.tsx>] [--out <abs>]\n"
          "\n"
          "Native Harmony equivalence gate (no local shell/python fallback).\n");
}

int native_verify_r2c_equivalence_harmony_native(const char *scripts_dir, int argc, char **argv, int arg_start) {
  ...
  if (scripts_dir == NULL || scripts_dir[0] == '\0') {
    fprintf(stderr, "[verify-r2c-harmony-native] missing scripts dir\n");
    return 2;
  }
  ...
  if (nr_path_join(out_default, sizeof(out_default), root, "build/r2c_equivalence_harmony_native") != 0) return 2;
  ...
  for (int i = arg_start; i < argc;) {
    ...
    fprintf(stderr, "[verify-r2c-harmony-native] unknown arg: %s\n", arg);
    return 2;
  }

  char out_dir_abs[PATH_MAX];
  if (to_abs_path(out_dir, out_dir_abs, sizeof(out_dir_abs)) != 0) {
    fprintf(stderr, "[verify-r2c-harmony-native] invalid out dir: %s\n", out_dir);
    return 2;
  }
  out_dir = out_dir_abs;
  ...
  if (nr_ensure_dir(compile_out) != 0 || nr_ensure_dir(native_out) != 0) {
    fprintf(stderr, "[verify-r2c-harmony-native] failed to create output directories\n");
    return 1;
  }

  setenv("STRICT_GATE_CONTEXT", "1", 1);
  setenv("R2C_TARGET_MATRIX", "harmony", 1);
  ...
  char err[512];
  if (nr_validate_compile_report(report_json, "truth_trace_manifest_harmony_path", project, err, sizeof(err)) != 0) {
    fprintf(stderr, "[verify-r2c-harmony-native] %s\n", err);
    return 1;
  }
  fprintf(stdout, "[verify-r2c-harmony-native] report fields ok\n");
  ...
  if (nr_path_join(entry_cheng, sizeof(entry_cheng), compile_out, "r2capp/src/entry.cheng") != 0 ||
    ...
    fprintf(stderr, "[verify-r2c-harmony-native] missing generated entry: %s\n", entry_cheng);
    return 1;
  }

  fprintf(stdout, "== r2c native equivalence: harmony native release build ==\n");
  char *run_argv[] = {
      "mobile_run_harmony",
      "--file",
      entry_cheng,
      "--name",
      "r2c_harmony_native_equivalence",
      "--out",
      native_out,
      NULL,
  };
  int run_rc = native_mobile_run_harmony(scripts_dir, 7, run_argv, 1);
  if (run_rc != 0) return run_rc;

  fprintf(stdout, "[verify-r2c-harmony-native] ok\n");
  return 0;
}
