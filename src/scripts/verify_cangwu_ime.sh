...
if [ -z "$CHENG_ROOT" ]; then
  echo "[verify-cangwu-ime] missing CHENG_ROOT" >&2
  exit 2
fi

CHENGC="${CHENGC:-$CHENG_ROOT/src/tooling/chengc.sh}"
if [ ! -x "$CHENGC" ]; then
  echo "[verify-cangwu-ime] missing chengc: $CHENGC" >&2
  exit 2
fi
...
if [ -z "$selected_driver" ]; then
  echo "[verify-cangwu-ime] no runnable backend driver found under CHENG_ROOT=$CHENG_ROOT" >&2
  exit 2
fi
...
export CHENG_PKG_ROOTS="${CHENG_PKG_ROOTS:-$HOME/.cheng-packages,$PKG_ROOT}"
frontend="${CHENG_CW_IME_FRONTEND:-stage1}"
full_mode="${CHENG_CW_IME_FULL:-0}"
compile_jobs="${CHENG_CW_IME_JOBS:-8}"
compile_incremental="${CHENG_CW_IME_INCREMENTAL:-1}"
compile_validate="${CHENG_CW_IME_VALIDATE:-0}"
stage1_skip_sem="${CHENG_CW_IME_STAGE1_SKIP_SEM:-1}"
stage1_skip_ownership="${CHENG_CW_IME_STAGE1_SKIP_OWNERSHIP:-1}"
stage1_generic_mode="${CHENG_CW_IME_GENERIC_MODE:-dict}"
stage1_generic_budget="${CHENG_CW_IME_GENERIC_SPEC_BUDGET:-0}"
...
test_main="${CHENG_CW_IME_TEST_MAIN:-}"
if [ -z "$test_main" ]; then
...
run_bundle="${CHENG_CW_IME_RUN_BUNDLE:-0}"
...
compile_obj() {
  ...
  (
    ...
    CHENG_BACKEND_JOBS="$compile_jobs" \
    CHENG_BACKEND_MULTI="${CHENG_BACKEND_MULTI:-0}" \
    CHENG_BACKEND_INCREMENTAL="$compile_incremental" \
    CHENG_BACKEND_WHOLE_PROGRAM=1 \
    CHENG_BACKEND_EMIT=obj \
    CHENG_BACKEND_FRONTEND="$frontend" \
    CHENG_BACKEND_INPUT="$input" \
    CHENG_BACKEND_OUTPUT="$output" \
    CHENG_BACKEND_VALIDATE="$compile_validate" \
...
run_test() {
  ...
  local timeout_s="${CHENG_CW_IME_TEST_TIMEOUT:-60}"
  set +e
  perl -e '
  ...
  ' "$timeout_s" env \
    CHENG_CW_IME_MAX_PHRASES="${CHENG_CW_IME_MAX_PHRASES:-100}" \
    CHENG_CW_IME_MAX_REVERSE="${CHENG_CW_IME_MAX_REVERSE:-200}" \
    "$bin"
  local rc=$?
  set -e
  if [ "$rc" = "124" ]; then
