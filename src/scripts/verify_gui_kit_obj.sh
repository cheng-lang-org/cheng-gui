...
if [ -z "$ROOT" ]; then
  echo "[verify-gui-kit-obj] missing ROOT" >&2
  exit 2
fi
...
if [ -z "${BACKEND_DRIVER:-}" ]; then
  ...
  if [ -z "$selected_driver" ]; then
    echo "[verify-gui-kit-obj] missing backend driver under ROOT=$ROOT" >&2
    exit 2
  fi
...
tmp_obj="$ROOT/chengcache/gui_kit_smoke.verify.o"
compile_log="$out_dir/gui_kit_smoke.compile.log"
mkdir -p "$out_dir"

(
  cd "$ROOT"
  DEFINES="${DEFINES:-macos,macosx}" sh "$CHENGC" "$ROOT/gui_kit_smoke_main.cheng" --emit-obj --obj-out:"$tmp_obj" --target:"$target"
) >"$compile_log" 2>&1
