...
if [ -z "$ROOT" ]; then
  echo "[verify-chromium-engine-obj] missing ROOT" >&2
  exit 2
fi
...
if [ -z "${BACKEND_DRIVER:-}" ]; then
  ...
  if [ -z "$selected_driver" ]; then
    echo "[verify-chromium-engine-obj] missing backend driver under ROOT=$ROOT" >&2
    exit 2
  fi
...
tmp_obj="$ROOT/chengcache/chromium_engine_smoke.verify.o"
compile_log="$out_dir/chromium_engine_smoke.compile.log"
mkdir -p "$out_dir"
reuse_engine_obj="${CHROMIUM_ENGINE_REUSE_OBJ:-0}"
if [ "$reuse_engine_obj" != "1" ] || [ ! -s "$obj" ]; then
  (
    cd "$ROOT"
    DEFINES="${DEFINES:-macos,macosx}" sh "$CHENGC" "$ROOT/chromium_engine_smoke_main.cheng" --emit-obj --obj-out:"$tmp_obj" --target:"$target"
  ) >"$compile_log" 2>&1
