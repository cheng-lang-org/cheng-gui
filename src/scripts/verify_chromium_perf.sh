...
if [ -z "$CHENG_ROOT" ]; then
  echo "[verify-chromium-perf] missing CHENG_ROOT" >&2
  exit 2
fi
...
if [ -z "${CHENG_BACKEND_DRIVER:-}" ]; then
  ...
  if [ -z "$selected_driver" ]; then
    echo "[verify-chromium-perf] missing backend driver under CHENG_ROOT=$CHENG_ROOT" >&2
    exit 2
  fi
...
mkdir -p "$out_dir"
obj="$CHENG_ROOT/chengcache/chromium_perf_smoke.runtime.o"
compile_log="$out_dir/chromium_perf_smoke.compile.log"
reuse_obj="${CHENG_CHROMIUM_PERF_REUSE_OBJ:-0}"
if [ "$reuse_obj" != "1" ] || [ ! -s "$obj" ]; then
  rm -f "$obj"
  if ! (
    cd "$CHENG_ROOT"
    CHENG_DEFINES="${CHENG_DEFINES:-macos,macosx}" sh "$CHENGC" "$main_src" --emit-obj --obj-out:"$obj" --target:"$target"
...
if [ "$host" = "Darwin" ]; then
  if [ "$runtime_exec" != "1" ]; then
    echo "[verify-chromium-perf] strict mode requires CHENG_CHROMIUM_RUNTIME_EXEC=1 on Darwin" >&2
    exit 1
  fi
  cc="${CC:-clang}"
  obj_sys="$CHENG_ROOT/chengcache/chromium_perf_smoke.system_helpers.runtime.o"
  obj_compat="$CHENG_ROOT/chengcache/chromium_perf_smoke.compat_shim.runtime.o"
  ...
  reuse_bin="${CHENG_CHROMIUM_PERF_REUSE_BIN:-0}"
  ...
  echo "[verify-chromium-perf] ok: runtime skipped host=$host"
fi
