...
out_dir="${R2C_EQ_IOS_OUT:-$ROOT/build/r2c_equivalence_ios_native}"
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -h|--help)
      echo "Usage: verify_r2c_equivalence_ios_native.sh [--project <abs>] [--entry </app/main.tsx>] [--out <abs>]"
      exit 0
      ;;
    *) echo "[verify-r2c-ios-native] unknown arg: $1" >&2; exit 2 ;;
  esac
done
...
export CHENG_IOS_REQUIRE_XCODE_BUILD="${CHENG_IOS_REQUIRE_XCODE_BUILD:-1}"

echo "== r2c native equivalence: ios compile =="
...
if [ ! -f "$report_json" ]; then
  echo "[verify-r2c-ios-native] missing report: $report_json" >&2
  exit 1
fi
...
for key in ("unsupported_syntax", "unsupported_imports", "degraded_features"):
    if len(data.get(key, []) or []) != 0:
        print(f"[verify-r2c-ios-native] {key} must be 0", file=sys.stderr)
        sys.exit(1)
if not bool(data.get("strict_no_fallback", False)):
    print("[verify-r2c-ios-native] strict_no_fallback != true", file=sys.stderr)
    sys.exit(1)
if bool(data.get("used_fallback", True)):
    print("[verify-r2c-ios-native] used_fallback != false", file=sys.stderr)
    sys.exit(1)
compiler_origin = str(data.get("compiler_report_origin", "") or "").strip()
if compiler_origin != "cheng-compiler":
    print(
        f"[verify-r2c-ios-native] compiler_report_origin != cheng-compiler: {compiler_origin}",
        file=sys.stderr,
    )
    sys.exit(1)
if bool(data.get("template_runtime_used", False)):
    print("[verify-r2c-ios-native] template_runtime_used != false", file=sys.stderr)
    sys.exit(1)
if str(data.get("semantic_compile_mode", "") or "") not in ("react-semantic-ir-node-compile", "react-semantic-ir-v1"):
    print("[verify-r2c-ios-native] semantic_compile_mode invalid", file=sys.stderr)
    sys.exit(1)
if str(data.get("semantic_mapping_mode", "") or "") != "source-node-map":
    print("[verify-r2c-ios-native] semantic_mapping_mode != source-node-map", file=sys.stderr)
    sys.exit(1)
semantic_count = int(data.get("semantic_node_count", 0) or 0)
if semantic_count <= 0:
    print("[verify-r2c-ios-native] semantic_node_count <= 0", file=sys.stderr)
    sys.exit(1)
required_paths = [
    "react_ir_path",
    "hook_graph_path",
...
for key in required_paths:
    ...
    if not p or not os.path.isfile(p):
        print(f"[verify-r2c-ios-native] missing {key}: {p}", file=sys.stderr)
        sys.exit(1)
...
if not semantic_map_path or not os.path.isfile(semantic_map_path):
    print(f"[verify-r2c-ios-native] missing semantic_node_map_path: {semantic_map_path}", file=sys.stderr)
    sys.exit(1)
if not semantic_runtime_map_path or not os.path.isfile(semantic_runtime_map_path):
    print(f"[verify-r2c-ios-native] missing semantic_runtime_map_path: {semantic_runtime_map_path}", file=sys.stderr)
    sys.exit(1)
if not semantic_render_nodes_path or not os.path.isfile(semantic_render_nodes_path):
    print(f"[verify-r2c-ios-native] missing semantic_render_nodes_path: {semantic_render_nodes_path}", file=sys.stderr)
    sys.exit(1)
if len(semantic_render_nodes_hash) != 64:
    print(f"[verify-r2c-ios-native] invalid semantic_render_nodes_hash: {semantic_render_nodes_hash}", file=sys.stderr)
    sys.exit(1)
if len(semantic_render_nodes_fnv64) != 16:
    print(f"[verify-r2c-ios-native] invalid semantic_render_nodes_fnv64: {semantic_render_nodes_fnv64}", file=sys.stderr)
    sys.exit(1)
if semantic_render_nodes_count <= 0:
    print("[verify-r2c-ios-native] semantic_render_nodes_count <= 0", file=sys.stderr)
    sys.exit(1)
if not generated_runtime_path or not os.path.isfile(generated_runtime_path):
    print(f"[verify-r2c-ios-native] missing generated_runtime_path: {generated_runtime_path}", file=sys.stderr)
    sys.exit(1)
...
if not isinstance(nodes, list) or len(nodes) != semantic_count:
    print("[verify-r2c-ios-native] semantic source map count mismatch", file=sys.stderr)
    sys.exit(1)
if not isinstance(runtime_nodes, list) or len(runtime_nodes) != semantic_count:
    print("[verify-r2c-ios-native] semantic runtime map count mismatch", file=sys.stderr)
    sys.exit(1)
if not all(isinstance(row, dict) for row in nodes):
    print("[verify-r2c-ios-native] semantic source map item type invalid (require object schema)", file=sys.stderr)
    sys.exit(1)
if not all(isinstance(row, dict) for row in runtime_nodes):
    print("[verify-r2c-ios-native] semantic runtime map item type invalid (require object schema)", file=sys.stderr)
    sys.exit(1)
...
if len(render_rows) != semantic_render_nodes_count:
    print(
        f"[verify-r2c-ios-native] semantic render rows mismatch: rows={len(render_rows)} report={semantic_render_nodes_count}",
        file=sys.stderr,
    )
    sys.exit(1)
if len(render_rows) < semantic_count:
    print(
        f"[verify-r2c-ios-native] semantic render rows too small: rows={len(render_rows)} semantic_count={semantic_count}",
        file=sys.stderr,
    )
    sys.exit(1)
import hashlib
...
if actual_render_hash != semantic_render_nodes_hash:
    print(
        f"[verify-r2c-ios-native] semantic render hash mismatch: report={semantic_render_nodes_hash} actual={actual_render_hash}",
        file=sys.stderr,
    )
    sys.exit(1)
...
if actual_render_fnv64 != semantic_render_nodes_fnv64:
    print(
        f"[verify-r2c-ios-native] semantic render fnv64 mismatch: report={semantic_render_nodes_fnv64} actual={actual_render_fnv64}",
        file=sys.stderr,
    )
    sys.exit(1)
...
if append_count < semantic_count:
    print(
        f"[verify-r2c-ios-native] generated runtime semantic nodes insufficient: append={append_count} expected={semantic_count}",
        file=sys.stderr,
    )
    sys.exit(1)
if "# appendSemanticNode(" in runtime_src:
    print("[verify-r2c-ios-native] generated runtime contains template semantic marker comments", file=sys.stderr)
    sys.exit(1)
...
if len(project_modules) < 5:
    print(f"[verify-r2c-ios-native] semantic source_module too small: {len(project_modules)}", file=sys.stderr)
    sys.exit(1)
missing = []
...
if missing_ratio > 0.05:
    print(
        f"[verify-r2c-ios-native] semantic source_module mismatch vs project files: missing={len(missing)}/{len(project_modules)} ratio={missing_ratio:.3f}",
        file=sys.stderr,
    )
    sys.exit(1)
print("[verify-r2c-ios-native] report fields ok")
PY

entry_cheng="$compile_out/r2capp/src/entry.cheng"
if [ ! -f "$entry_cheng" ]; then
  echo "[verify-r2c-ios-native] missing generated entry: $entry_cheng" >&2
  exit 1
fi

echo "== r2c native equivalence: ios native release build =="
/Users/lbcheng/.cheng-packages/cheng-mobile/scripts/build_native_ios_release.sh --file:"$entry_cheng" --name:r2c_ios_native_equivalence --out:"$native_out"

echo "[verify-r2c-ios-native] ok"
