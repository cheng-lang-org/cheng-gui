...
if [ -z "$CHENG_ROOT" ]; then
  echo "[verify-chromium-network-features] missing CHENG_ROOT" >&2
  exit 2
fi
...
if [ -z "${CHENG_BACKEND_DRIVER:-}" ]; then
  ...
  if [ -z "$selected_driver" ]; then
    echo "[verify-chromium-network-features] missing backend driver under CHENG_ROOT=$CHENG_ROOT" >&2
    exit 2
  fi
...
mkdir -p "$out_dir"
obj="$CHENG_ROOT/chengcache/chromium_network_features_smoke.runtime.o"
compile_log="$out_dir/chromium_network_features_smoke.compile.log"
reuse_obj="${CHENG_CHROMIUM_NETWORK_REUSE_OBJ:-0}"
...
if [ "$host" = "Darwin" ]; then
  if [ "$runtime_exec" != "1" ]; then
    echo "[verify-chromium-network-features] strict mode requires CHENG_CHROMIUM_RUNTIME_EXEC=1 on Darwin" >&2
    exit 1
  fi
  ...
  cc="${CC:-clang}"
  obj_sys="$CHENG_ROOT/chengcache/chromium_network_features_smoke.system_helpers.runtime.o"
  obj_compat="$CHENG_ROOT/chengcache/chromium_network_features_smoke.compat_shim.runtime.o"
  ...
  reuse_bin="${CHENG_CHROMIUM_NETWORK_REUSE_BIN:-0}"
...
PY
  ...
  export CHENG_NET_HTTP_URL="http://127.0.0.1:${http_port}/index.html"
  export CHENG_NET_HTTPS_URL="https://127.0.0.1:${https_port}/index.html"
  ...
  echo "[verify-chromium-network-features] ok: runtime skipped host=$host"
fi
