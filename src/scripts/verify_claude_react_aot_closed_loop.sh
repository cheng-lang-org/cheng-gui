...
if [ -z "$ROOT" ]; then
  echo "[verify-claude-aot] missing ROOT" >&2
  exit 2
fi
...
if [ -z "${BACKEND_DRIVER:-}" ]; then
  ...
  if [ -z "$selected_driver" ]; then
    echo "[verify-claude-aot] missing backend driver under ROOT=$ROOT" >&2
    exit 2
  fi
...
cc="${CC:-clang}"
obj_sys="$ROOT/chengcache/claude_aot.system_helpers.runtime.o"
obj_compat="$ROOT/chengcache/claude_aot.compat_shim.runtime.o"
...
compiler_obj="$ROOT/chengcache/claude_aot.compiler.runtime.o"
...
if [ "${R2C_LEGACY_UNIMAKER:-0}" != "0" ]; then
  echo "[verify-claude-aot] strict mode: R2C_LEGACY_UNIMAKER must be 0" >&2
  exit 2
fi
...
smoke_obj="$ROOT/chengcache/claude_aot.smoke.runtime.o"
...
export CLAUDE_HTTP_URL="http://127.0.0.1:${http_port}/index.html"
export CLAUDE_HTTPS_URL="https://127.0.0.1:${https_port}/index.html"
