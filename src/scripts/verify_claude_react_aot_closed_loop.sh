...
if [ -z "$CHENG_ROOT" ]; then
  echo "[verify-claude-aot] missing CHENG_ROOT" >&2
  exit 2
fi
...
if [ ! -x "$CHENGC" ]; then
  echo "[verify-claude-aot] missing chengc: $CHENGC" >&2
  exit 2
fi

host="$(uname -s)"
if [ "$host" != "Darwin" ]; then
  echo "[verify-claude-aot] skip: host=$host (runtime smoke currently macOS-only)"
  exit 0
fi

for bin in python3 openssl curl; do
  if ! command -v "$bin" >/dev/null 2>&1; then
    echo "[verify-claude-aot] missing dependency: $bin" >&2
    exit 2
  fi
done
...
if [ -z "${CHENG_BACKEND_DRIVER:-}" ]; then
  ...
  if [ -z "$selected_driver" ]; then
    echo "[verify-claude-aot] missing backend driver under CHENG_ROOT=$CHENG_ROOT" >&2
    exit 2
  fi
...
if [ -z "$target" ]; then
  echo "[verify-claude-aot] failed to detect host target" >&2
  exit 2
fi
...
if [ ! -f "$aot_src" ] || [ ! -f "$smoke_src" ]; then
  echo "[verify-claude-aot] missing sources under $ROOT" >&2
  exit 1
fi

fixture_root="$ROOT/tests/claude_fixture"
if [ ! -f "$fixture_root/index.html" ] || [ ! -f "$fixture_root/app/main.tsx" ]; then
  echo "[verify-claude-aot] missing fixture under: $fixture_root" >&2
  exit 1
fi

out_dir="$ROOT/build/claude_aot_closed_loop"
mkdir -p "$out_dir"
...
cc="${CC:-clang}"
obj_sys="$CHENG_ROOT/chengcache/claude_aot.system_helpers.runtime.o"
obj_compat="$CHENG_ROOT/chengcache/claude_aot.compat_shim.runtime.o"
...
compile_and_link() {
  ...
  if ! (
    ...
    echo "[verify-claude-aot] compile failed: $input" >&2
    sed -n '1,120p' "$compile_log" >&2
    exit 1
  fi
  if [ ! -s "$obj" ]; then
    echo "[verify-claude-aot] compile failed: missing obj: $obj" >&2
    sed -n '1,120p' "$compile_log" >&2
    exit 1
  fi
...
compiler_obj="$CHENG_ROOT/chengcache/claude_aot.compiler.runtime.o"
compiler_bin="$out_dir/claude_aot_compiler_macos"
compiler_log="$out_dir/claude_aot_compiler.compile.log"
...
if [ "${CHENG_R2C_LEGACY_UNIMAKER:-0}" != "0" ]; then
  echo "[verify-claude-aot] strict mode: CHENG_R2C_LEGACY_UNIMAKER must be 0" >&2
  exit 2
fi

compiler_run_log="$out_dir/claude_aot_compiler.run.log"
set +e
"$compiler_bin" >"$compiler_run_log" 2>&1
compiler_rc=$?
set -e
if [ "$compiler_rc" -ne 0 ]; then
  echo "[verify-claude-aot] compiler failed rc=$compiler_rc" >&2
  if [ -f "$out_pkg/r2capp_compiler_error.txt" ]; then
    echo "[verify-claude-aot] compiler error:" >&2
...
if [ ! -f "$out_pkg/cheng-package.toml" ] || [ ! -f "$out_pkg/src/entry.cheng" ]; then
  echo "[verify-claude-aot] compiler did not generate r2capp package under: $out_pkg" >&2
  exit 1
fi
if [ ! -f "$out_pkg/r2capp_compile_report.json" ]; then
  echo "[verify-claude-aot] missing compile report: $out_pkg/r2capp_compile_report.json" >&2
  exit 1
fi
...
if data.get("used_fallback", False):
    print("[verify-claude-aot] report used_fallback=true", file=sys.stderr)
    sys.exit(1)
if int(data.get("compiler_rc", 0)) != 0:
    print(f"[verify-claude-aot] report compiler_rc != 0: {data.get('compiler_rc')}", file=sys.stderr)
    sys.exit(1)
for key in ("unsupported_syntax", "unsupported_imports", "degraded_features"):
    items = data.get(key, [])
    if isinstance(items, list) and len(items) > 0:
        print(f"[verify-claude-aot] {key} != 0: {len(items)}", file=sys.stderr)
        sys.exit(1)
...
smoke_obj="$CHENG_ROOT/chengcache/claude_aot.smoke.runtime.o"
smoke_bin="$out_dir/claude_aot_smoke_macos"
smoke_log="$out_dir/claude_aot_smoke.compile.log"
...
if [ ! -s "$http_port_file" ] || [ ! -s "$https_port_file" ]; then
  echo "[verify-claude-aot] fixture server failed to start" >&2
...
export CHENG_CLAUDE_HTTP_URL="http://127.0.0.1:${http_port}/index.html"
export CHENG_CLAUDE_HTTPS_URL="https://127.0.0.1:${https_port}/index.html"
export CHENG_GUI_TEST_INSECURE_TLS=1

run_log="$out_dir/claude_aot_smoke_macos.run.log"
set +e
"$smoke_bin" >"$run_log" 2>&1
run_rc=$?
set -e
if [ "$run_rc" -ne 0 ]; then
  echo "[verify-claude-aot] runtime failed rc=$run_rc" >&2
  sed -n '1,120p' "$run_log" >&2
  exit 1
fi

echo "[verify-claude-aot] ok: $smoke_bin"
