...
export default function PublishSellPage({ onClose }: PublishSellPageProps) {
    ...
    const categories = [t.pubSell_property, t.pubSell_vehicle, t.pubSell_land, t.pubSell_shop, t.pubSell_equipment, t.pub_other];
    ...
    const handlePublish = async () => {
        ...
        try {
            ...
            const summary = `${title}${category ? ` · ${category}` : ''}${price ? ` · ¥${price}` : ''}${negotiable ? ` (${t.pub_negotiable})` : ''}`;
            await publishDistributedContent({
                publishCategory: 'sell',
                ...
                extra: {
                    sellMeta: { title, description, category, location: locationInput, contact, negotiable },
    ...
    return (
        <div className="fixed inset-0 bg-white z-50 flex flex-col">
            <header className="flex items-center justify-between px-4 py-3 border-b border-gray-200">
                ...
                <h1 className="font-semibold text-lg">{t.pubSell_title}</h1>
                <button onClick={handlePublish} disabled={!canPublish || isPublishing} className={`px-5 py-2 rounded-full font-medium transition-colors ${canPublish && !isPublishing ? 'bg-purple-500 text-white hover:bg-purple-600' : 'bg-gray-200 text-gray-400'}`}>{isPublishing ? t.common_loading : t.pub_publish}</button>
            </header>
            {publishError && (
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-5">
                <div>
                    <h3 className="text-sm font-medium text-gray-700 mb-2 flex items-center gap-2"><Camera size={16} />{t.pubSell_image}</h3>
                ...
                <div>
                    <h3 className="text-sm font-medium text-gray-700 mb-2 flex items-center gap-1"><Tag size={14} />{t.pubSell_type}</h3>
                ...
                <div className="flex gap-2">
                    ...
                    <button onClick={() => setNegotiable(!negotiable)} className={`px-4 rounded-xl border ${negotiable ? 'bg-purple-50 border-purple-200 text-purple-600' : 'bg-gray-50 border-gray-200 text-gray-500'}`}>{t.pub_negotiable}</button>
                </div>
                <div>
                    <h3 className="text-sm font-medium text-gray-700 mb-2 flex items-center gap-1"><Phone size={14} />{t.pub_contact}</h3>
                    <input type="text" placeholder={t.pub_contactPh} value={contact} onChange={(e) => setContact(e.target.value)} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl" />
                </div>
