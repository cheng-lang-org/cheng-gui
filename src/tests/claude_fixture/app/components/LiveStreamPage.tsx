...
export default function LiveStreamPage({ onClose }: LiveStreamPageProps) {
    ...
    const [streamKey] = useState(() => `stream-${Date.now()}`);
    const [isStarting, setIsStarting] = useState(false);
    const [startError, setStartError] = useState('');

    const handleStartLive = async () => {
        if (!title.trim() || isStarting) {
            return;
        }
        setStartError('');
        setIsStarting(true);
        try {
            await libp2pService.upsertLivestreamConfig(streamKey, {
                title: title.trim(),
                codec: 'h264',
                fps: 30,
                bitrateKbps: 1200,
            ...
            await publishDistributedContent({
                publishCategory: 'live',
                type: 'video',
                content: `直播开始：${title.trim()}`,
            });
            setIsLive(true);
        } catch (error) {
            setStartError(getPublishLocationErrorMessage(t, error));
        } finally {
            setIsStarting(false);
        }
    };
    ...
    useEffect(() => {
        ...
        const frameTimer = setInterval(() => {
            ...
            void libp2pService.publishLivestreamFrame(streamKey, payload);
        }, 1000);
        return () => clearInterval(frameTimer);
    }, [isCameraOn, isLive, isMicOn, streamKey, title]);
            ...
            <div className="fixed inset-0 bg-gray-900 z-50 flex flex-col">
                ...
                <div className="p-6 space-y-4">
                    ...
                    <button
                        ...
                        className={`w-full py-4 rounded-full font-semibold text-lg transition-colors ${title.trim() && !isStarting
                            ? 'bg-gradient-to-r from-red-500 to-red-600 text-white hover:from-red-600 hover:to-red-700'
                            : 'bg-gray-700 text-gray-500'
                            }`}
                    >
                        {isStarting ? t.common_loading : '开始直播'}
                    </button>
                </div>
            </div>
        );
